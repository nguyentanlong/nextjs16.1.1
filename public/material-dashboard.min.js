...
document.addEventListener('click', function (event) {
    ...
    if (body.classList.contains(className)) {
        // Kiểm tra xem click có nằm ngoài sidebar và ngoài 2 nút toggle không
        if (
            !sidenav.contains(event.target) &&
            !iconNavbarSidenav.contains(event.target) &&
            !iconSidenav.contains(event.target)
        ) {
            // Đóng sidebar
